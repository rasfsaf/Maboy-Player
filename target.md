...
* [x] Recently played — реализовано экраном синхронизируемой истории воспроизведения.
* [x] Recently added — локальные импорты получают `added_at` и выводятся отдельной секцией библиотеки.
...
* [x] включение mix не перезапускает и не переключает текущий трек;
* [x] в Player показывается уже перемешанная активная последовательность;
* [x] текущая позиция закреплена, будущие треки можно выбирать, удалять и
  переставлять drag-and-drop.

**Было:** включение mix повторно запускало текущий трек через замену источника,
а фактический shuffled-порядок был скрыт от пользователя.

**Стало:** меняется только порядок будущих элементов. Полный Player показывает
этот порядок и позволяет редактировать его без скрытого повторного shuffle.
Миграция не требуется: серверный API и формат синхронизируемых данных не менялись.
...
* volume;
* Queue button.
* editable active Queue below the track card.
...
* [x] **playback** — **Выполнено** (локальные файлы воспроизводятся через `just_audio`; доступны play/pause, seek, previous/next, persistent mini player и полная панель плеера);
...
* [x] **search** — **Выполнено** (локальный поиск по названию трека, исполнителю и альбому);
* [x] **favorites** — **Выполнено** (offline-first избранное с синхронизацией между устройствами через `favorites.set`);
* [x] **history** — **Выполнено** (история запуска треков с `track_id`, `started_at`, `completed`, `device_id`, локальным хранением и синхронизацией через `history.add`).

### Playlists

* [x] **create** — **Выполнено** (создание плейлистов локально и через мутацию `playlist.upsert`);
* [x] **rename** — **Выполнено** (переименование через меню плейлиста и offline-first `playlist.upsert`);
* [x] **delete** — **Выполнено** (удаление с подтверждением через синхронизируемую операцию `playlist.delete`);
* [x] **add/remove tracks** — **Выполнено** (добавление и удаление отдельных треков через `playlist.set_tracks`);
* [x] **reorder tracks** — **Выполнено** (ручной drag & drop порядок треков внутри плейлиста);
* [x] **reorder playlists** — **Выполнено** (drag & drop плейлистов с синхронизацией `sort_key`).

### Queue

* [x] **Play next** — **Выполнено** (контекстное действие трека помещает его в начало очереди);
...
* [x] **clear** — **Выполнено** (кнопка очистки синхронизирует пустую очередь);
* [x] **editable shuffle** — **Выполнено** (очередь перемешивается, сохраняется и остаётся редактируемой).

### Sync

* [x] **multi-device** — **Выполнено** (архитектура синхронизации изолирована по `user_id` и рассчитана на несколько устройств);
* [x] **VPS** — **Выполнено** (FastAPI сервер развёрнут в docker compose на порту `18181`; `https://maboy.dofic.site` обслуживается через Nginx с Let's Encrypt и WebSocket Upgrade);
* [x] **offline-first** — **Выполнено** (локальное сохранение состояния, буфер `pending` мутаций и автоматическая досылка при наличии сети);
* [x] **realtime updates** — **Выполнено** (авторизованный WebSocket `/sync/events` сообщает о новых версиях; клиент автоматически выполняет pull и переподключается после обрыва).

### UI

* [~] **polished black/dark design** — **Частично** (задана тёмная тема с фоном `#0d0d0f` и добавлен Persistent Mini Player, но пока нет обложек и целостного кастомного стиля).
